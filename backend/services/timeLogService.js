...
exports.startBreak = async (empId, dateInput, breakType) => {
  console.log('startBreak date input:', dateInput, 'empId:', empId, 'breakType:', breakType);

  const dateObj = dateInput instanceof Date ? dateInput : new Date(dateInput);
  if (isNaN(dateObj)) {
    console.error('Invalid date in startBreak:', dateInput);
  ...
  const breakFields = {
    break1: ['break1StartTime', 'break1EndTime'],
    break2: ['break2StartTime', 'break2EndTime'],
  };

  const fields = breakFields[breakType];
  if (!fields) {
  ...
  if (timeLog[endField]) {
    throw new Error(`${breakType} already completed`);
  }
  if (timeLog[startField]) {
    throw new Error(`${breakType} already started`);
  }
  if (breakType === 'break2' && !timeLog.break1EndTime) {
    throw new Error('Break 1 must be completed first');
  }

  timeLog[startField] = new Date();
  timeLog.updatedAt = new Date();
  await timeLog.save();

  console.log('Updated TimeLog with', breakType, 'startTime:', timeLog[startField].toISOString());
  return timeLog;
};
...
exports.endBreak = async (empId, dateInput, breakType, breakId) => {
  console.log('endBreak date input:', dateInput);
  ...
  // Handle bio breaks separately
  if (breakType === 'bio') {
    const bioBreak = await BioBreak.findById(breakId);
    if (!bioBreak) {
      throw new Error('Bio Break not found');
    }
    bioBreak.endTime = new Date();
    bioBreak.duration = (bioBreak.endTime - bioBreak.startTime) / 60000; // minutes
    await bioBreak.save();
    return bioBreak;
  }

  // Retrieve the day's time log
  const timeLog = await TimeLog.findOne({ empId, date: startOfDay });
  if (!timeLog || timeLog.logoutTime) {
    throw new Error('No active session found');
  }

  // Validate and end the specified break
  const breakFields = {
    break1: ['break1StartTime', 'break1EndTime'],
    break2: ['break2StartTime', 'break2EndTime'],
  };
  ...
  const [startField, endField] = fields;
  if (!timeLog[startField] || timeLog[endField]) {
    throw new Error(`${breakType} not active`);
  }

  timeLog[endField] = new Date();
  timeLog.updatedAt = new Date();
  await timeLog.save();
